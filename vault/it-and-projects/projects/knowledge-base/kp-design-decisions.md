...
## ADR-009: Plus Jakarta Sans replaces Lora (v2.0 redesign)

**Date:** 2026-03-03 (Phase 1)
**Status:** Decided

**Context:**
v2.0 visual redesign aligned the Knowledge Base aesthetic with Applyr (same design system). ADR-006 (Lora serif body) was revisited.

**Decision:** Replace Lora + DM Sans with Plus Jakarta Sans across the entire UI. JetBrains Mono retained for code and metadata.

**Reasons:**
- Consistent design language across the SS42 tool suite (Applyr uses Plus Jakarta Sans)
- Plus Jakarta Sans at 16px/1.6 reads cleanly for documentation — less stylised than Lora but more versatile
- Simplifies font loading: two families (Plus Jakarta Sans + JetBrains Mono) instead of three
- The indigo accent colour system renders better against a neutral sans-serif

**Trade-off:** Loss of the deliberate serif/sans contrast from v1. Accepted — cohesion across apps outweighs individual expressiveness.
...
**Update (Phase 5, 2026-03-03):** A Mermaid diagram toggle was added alongside the filterable table. Clicking "Diagram" renders a `flowchart LR` Mermaid chart from the filtered relationship data using the already-present Mermaid.js CDN dependency. This preserves the table as primary while delivering a visual alternative without adding a new library. The D3/Cytoscape graph decision (full interactive graph) remains deferred.
